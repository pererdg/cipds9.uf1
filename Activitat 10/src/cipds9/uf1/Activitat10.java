package cipds9.uf1;

import java.security.KeyFactory;
import java.security.PrivateKey;
import java.security.Signature;
import java.security.spec.PKCS8EncodedKeySpec;
import java.util.Base64;

/**
 * Signa document amb les dades de l'exercici 8
 * 
 * Activitat 8 - POSSIBLE SORTIDA EN BASE 64
 * Clau privada: 
 * Clau pública: 
 * Wrapped text: 
 * Wrapped clau simètrica: 
 * 
 * Activitat 8 - POSSIBLE SORTIDA EN BASE 64 (CRYPTOLIB)
 * Clau pública: 
 * Clau privada: 
 * Wrapped text:
 * Wrapped clau simètrica: 
 * 
 * RESULTAT
 * 
 *
 * RESULTAT CRYPTOLIB
 * 
 *
 * @author pererdg
 */
public class Activitat10 {

	public static void main(String[] args) throws Exception {
		
		// Dades d'activitat 8
		String text = "Escriu un programa que generi les claus pública i privada amb una longitud de 2048 bits. Un cop creades, ha d’imprimir-les i comprovar-ne la longitud.\r\n"
			+ "Transforma les claus a base 64 i torna a imprimir-les.\r\n"
			+ "Amb aquestes claus, utilitzant el xifratge híbrid, ha de xifrar el text d’aquest enunciat (sense incloure el títol). El xifratge ha de ser amb AES i RSA i finalment escriure per pantalla tot el resultat del xifrat híbrid transformat a base 64.\r\n";
		String priv64 = "";
		
		// Recuperar clau privada
		PrivateKey priv = KeyFactory.getInstance("RSA").generatePrivate(new PKCS8EncodedKeySpec(Base64.getDecoder().decode(priv64)));
		
		// Signar i imprimir
		byte[] signat = signar(priv, text);
		System.out.println(Base64.getEncoder().encodeToString(signat));
		cryptoLib();
	}
		
	private static void cryptoLib() throws Exception {
		System.out.println("-- CRYPTOLIB --");
		// Dades d'activitat 8
		String text = "Escriu un programa que generi les claus pública i privada amb una longitud de 2048 bits. Un cop creades, ha d’imprimir-les i comprovar-ne la longitud.\r\n"
			+ "Transforma les claus a base 64 i torna a imprimir-les.\r\n"
			+ "Amb aquestes claus, utilitzant el xifratge híbrid, ha de xifrar el text d’aquest enunciat (sense incloure el títol). El xifratge ha de ser amb AES i RSA i finalment escriure per pantalla tot el resultat del xifrat híbrid transformat a base 64.\r\n";
		String priv64 = "";
		
		// Recuperar clau privada
		PrivateKey priv = KeyFactory.getInstance("RSA").generatePrivate(new PKCS8EncodedKeySpec(Crypto.decodeBase64(priv64)));
		
		// Signar i imprimir
		byte[] signat = CryptoAsym.signText(priv, text);
		System.out.println(Crypto.encodeBase64(signat));
	}
	
	/**
	 * Signa dades amb SHA256/RSA
	 * @param priv Clau privada
	 * @param text Text a signar
	 * @return
	 * @throws Exception
	 */
	public static byte[] signar(PrivateKey priv, String text) throws Exception {
		Signature signer = Signature.getInstance("SHA256withRSA");
		signer.initSign(priv);
		signer.update(text.getBytes("UTF-8"));
		return signer.sign();
	}
}
